...
int temps(int numCores)
{
	...
	if(!dryRun)
	{
		if(check == 2)
		{
			system("sudo " IPMITOOL_PATH " raw 0x3a 0x07 0x01 0xff 0x01 && sudo" IPMITOOL_PATH " raw 0x3a 0x07 0x02 0xff 0x01");
		}
		else if(check == 1)
		{
			system("sudo " IPMITOOL_PATH " raw 0x3a 0x07 0x01 0xff 0x00 &&  sudo " IPMITOOL_PATH " raw 0x3a 0x07 0x02 0xff 0x00");
		}
		else
		{
			system("sudo " IPMITOOL_PATH "raw 0x3a 0x07 0x02 0x30 0x01 && sudo " IPMITOOL_PATH " raw 0x3a 0x07 0x01 0x30 0x01");
		}
	}
	pclose(fp);
	return check;
