...
int temps(int numCores)
{
	...
	if(!dryRun)
	{
		if(check == 2)
		{
			const char *command = " " IPMITOOL_PATH " raw 0x3a 0x07 0x01 0xff 0x01 &&  " IPMITOOL_PATH " raw 0x3a 0x07 0x02 0xff 0x01";
			system(command);
		}
		else if(check == 1)
		{
			const char *command = " " IPMITOOL_PATH " raw 0x3a 0x07 0x01 0xff 0x00 &&   " IPMITOOL_PATH " raw 0x3a 0x07 0x02 0xff 0x00";
			system(command);
		}
		else
		{
			const char *command = " " IPMITOOL_PATH " raw 0x3a 0x07 0x02 0x30 0x01 &&  " IPMITOOL_PATH " raw 0x3a 0x07 0x01 0x30 0x01";
			system(command);
		}
	}
	pclose(fp);
...
//		printf("fan %i\n", fan);
	while(1)
	{
		...
		if(verbose)
		{
			...
			if(temp == 0)
			{
				printf("'ipmitool raw 0x3a 0x07 0x02 0x34 0x01 && sudo ipmitool raw 0x3a 0x07 0x01 0x34 0x01'\n");
